...
/**
 ...
 The ReplaceGreater implements the Command interface and represents a command to replace a worker in the collection with a new one if it is greater than the given one.
...
public class ReplaceGreater implements Command{
    ...
    /**
     ...
     Replaces a worker in the collection with a new one if it is greater than the given one.
     The new worker is obtained from user input using ConsoleCreation class.
     If the new worker is greater than the existing one, the existing worker is removed from the collection
